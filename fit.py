...
def continue_existing_run(run_to_continue: Path):
    ...
    fit_run_name = f'{match.group(1)}_1e-6_temperature_cont'
...
if __name__ == '__main__':
    ...
    for path in Path('data/mb20208/runs').glob('*'):
        if 'moa' in path.name and 'mcmc0' in path.name and 'cont' not in path.name and not path.joinpath('run_2.in').exists():
